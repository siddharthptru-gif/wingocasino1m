...
app.post('/api/game/deposit-request', authenticateToken, (req, res) => {
  try {
    ...
    io.emit('new-deposit-request', depositRequest);
...
app.get('/api/game/my-deposits', authenticateToken, (req, res) => {
  try {
    const deposits = readDB('deposits');
    const myDeposits = deposits.filter(d => d.userId === req.user.userId);
    res.json(myDeposits);
  } catch (error) {
...
app.get('/api/game/my-withdrawals', authenticateToken, (req, res) => {
  try {
    const withdrawals = readDB('withdrawals');
    const myWithdrawals = withdrawals.filter(w => w.userId === req.user.userId);
...
app.get('/api/admin/bets', authenticateToken, (req, res) => {
  try {
    const bets = readDB('bets');
    res.json(bets.slice(-100)); // Last 100 bets
  } catch (error) {
...
function generateGameResult() {
  // Generate random number between 0-9 (correct Wingo game)
  const winningNumber = Math.floor(Math.random() * 10);
  
  // Determine Big or Small
  // Big = 5,6,7,8,9 | Small = 0,1,2,3,4
  const bigSmallResult = winningNumber >= 5 ? 'big' : 'small';
  
  // Determine Color based on Wingo rules:
  // Green: 1,3,7,9
  // Red: 2,4,6,8
  // Violet: 0,5
  let winningColor;
