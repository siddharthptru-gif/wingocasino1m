...
class GameManager {
    ...
    setupEventListeners() {
        ...
        document.getElementById('myBetsTab').addEventListener('click', () => this.switchTab('game', 'myBets'));
    ...
    setupSocket() {
        ...
        this.socket.on('bet-result', (data) => {
            if (data.won) alert(`Congratulations! You won ₹${data.pay.toFixed(2)}`);
            this.fetchProfile();
            this.fetchMyBets();
        });
    ...
    async fetchMyBets() {
        // Implementation for fetching specific user bets if needed
        // For now, let's keep it simple or assume we fetch them from a generic history
    }
    ...
    async handleDeposit() {
        ...
        const upi = document.getElementById('depositUpi').value;
